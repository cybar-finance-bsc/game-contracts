...
contract Lottery is Ownable, Initializable, Testable {
    ...
    constructor(
        ...
        require(
            _bucketOneMaxNumber != 0 && _bucketTwoMaxNumber != 0,
    ...
    function updateBuckets(
        ...
        require(
            _bucketOneMax != 0 && _bucketTwoMax != 0,
            "Bucket range cannot be 0"
        );
        require(_bucketOneMax < _bucketTwoMax, "Bucket one must be smaller");
        require(
            _discountForBucketOne < _discountForBucketTwo &&
                _discountForBucketTwo < _discountForBucketThree,
            "Discounts must increase"
        ...
        onlyOwner
    {
        ...
        allLotteries_[_lotteryId].numberDistribution = _calculateHistogram(
            _lotteryId
        );
    ...
    function batchBuyLottoTicket(
        ...
        cybar_.transferFrom(msg.sender, address(this), costWithDiscount);
        ...
        notContract
    {
        ...
        for (uint256 i = 0; i < _tokeIds.length; i++) {
            ...
            // Removing the prize amount from the pool
            allLotteries_[_lotteryId].prizePoolInCybar = allLotteries_[
                _lotteryId
            ].prizePoolInCybar.sub(prizeAmount);
    ...
    function _discount(uint256 lotteryId, uint256 _numberOfTickets)
        internal
        view
        returns (uint256 discountAmount)
    {
        ...
        } else if (_numberOfTickets < bucketTwoMax_) {
        ...
        returns (uint256)
    {
        ...
        uint256 numberOfWinners = allLotteries_[_lotteryId].numberDistribution[
            _noOfMatching - 1
        ];
        // Timesing the percentage one by the pool
        /* prize = allLotteries_[_lotteryId].prizePoolInCybar.mul(perOfPool).div(1); */
        prize = allLotteries_[_lotteryId].prizePoolInCybar.mul(perOfPool).div(
            numberOfWinners
        );
    ...
    function _setWinningNumbers(uint256 _randomNumber)
        internal
        view
    ...
    function _calculateHistogram(uint256 _lotteryId)
        internal
        view
        returns (uint256[] memory)
    {
        ...
        uint16[] memory winningNumbers = allLotteries_[_lotteryId]
            .winningNumbers;
        ...
        for (uint256 i = idFirstTicket_; i <= totalSupply; i++) {
            ...
            if (matchingNumbers != 0) {
                numberDistribution[matchingNumbers - 1] =
                    numberDistribution[matchingNumbers - 1] +
                    1;
            }
        }
